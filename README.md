...
Each extraction also receives up to 20 recent human canon rulings from that campaign. Accepted edits show the table's preferred wording, while incorrect, secret-leak, and not-useful outcomes tell the extractor which proposal patterns not to repeat. Rulings remain quoted context rather than instructions, and the same strict citation and visibility checks still apply to every generated proposal.

Human rulings can also be inspected as a versioned, local evaluation set. `npm run eval:feedback` reads `DATABASE_PATH` (or `data/wayfarer.sqlite`) and writes JSON to stdout with canon acceptance, edit, dispute, and rejection rates plus continuity usefulness, error, and secret-leak rates, both overall and by generator version. Add `-- --campaign ID` to scope one campaign or `-- --output PATH` to create a new file; an existing output file is never overwritten. The export excludes player and campaign names but contains exact cited campaign text, so keep it private and do not commit it.
