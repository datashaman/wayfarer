...
Comparable live-model results can be appended with `npm run eval:record`; see `docs/ai-evaluation.md` for arguments and release thresholds. Living Canon shows campaign readiness against those feedback thresholds. Readiness can only enable draft preparation—never automatic publication or canon changes.
