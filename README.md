...
Comparable live-model results can be appended with `npm run eval:record`; see `docs/ai-evaluation.md` for arguments and release thresholds. The GM-only AI evaluation ledger compares human verdicts by generator version, shows secret-leak and regression warnings, tracks release gates, and preserves campaign-scoped live-check history. Readiness can only enable draft preparation—never automatic publication or canon changes.

`npm run eval:all` runs and records the canon, continuity, contradiction, and recap live suites together. Once a campaign passes every release gate, a GM may opt into post-session preparation from **Table tools**. Closing a session then queues only the selected private drafts—canon suggestions, a continuity brief, and/or a recap. Readiness is checked again for every run, and no result is accepted or published automatically.

**Table tools** also holds four bounded campaign workflows. Character recollection answers cite only canon readable by the requesting seat. The intent studio offers private, editable phrasing grounded in that player’s own messages and never sends it. Negotiated house rules preserve the source rule, interpretation, table ruling, revision reason, and retirement history. GM-private faction clocks receive counterfactual proposals with explicit assumptions and advance only after acceptance. Optional spotlight reports count only text sent after each seat opts in; they never record voice or infer emotion, attention, engagement, or intent.
