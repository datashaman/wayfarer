...
`npm run eval:all` runs and records every registered live suite together. Campaign-intelligence surfaces can also be exercised independently with their `eval:*` commands, including campaign seed, character concepts, in-play material, and adventure continuation. Once a campaign passes every release gate, a GM may opt into post-session preparation from **Table tools**. Closing a session then queues only the selected private drafts—canon suggestions, a continuity brief, and/or a recap. Readiness is checked again for every run, and no result is accepted or published automatically.
...
Once the world and at least one character exist, a GM uses the top-level **Scene** folio to prepare actual play. The saved opening crisis pre-fills the moment, framing, and stakes; character connections preselect likely locations and NPCs. The GM keeps an editable, revision-protected session folio containing the present cast, stage, first choice, clues or discoveries, likely complications, and questions the table will play to answer. None prescribe an outcome. **Begin play** is available only after that preparation is saved; it snapshots the complete GM-private folio, creates the campaign’s single active scene and an immutable threshold marker in the `in-character` transcript, then takes the GM directly there. Ordinary character messages continue beneath that framing. Resolving the scene requires a concrete statement of what changed, adds a matching transcript marker, and frees the table to prepare the next scene. Scene markers are human-authored campaign records, not AI narration.

After a resolved scene, the same folio can **Draft from play**. Adventure continuation receives bounded current World state, resolved outcomes, open hooks, unresolved pressure, and public character drives and beliefs as untrusted fiction. It must copy only saved cast and stage IDs and cite the exact surviving thread IDs supporting its proposed opening, stakes, first choice, discoveries, complications, and questions. The private continuation braid previews those sources before **Use as preparation** populates the ordinary editable folio; no draft is stored until the GM separately saves preparation. Run `npm run eval:adventure-continuation` for its bounded live fixture.

When play leaves the prepared path, the active Scene folio can draft one NPC, place, complication, possible consequence, rumour, or treasure from a short GM prompt. The model receives the established World and exact active-scene snapshot as untrusted fiction, returns strict editable fields, and stores no response through the API. Nothing enters the campaign until the GM chooses **Keep in campaign**. Kept NPCs and places become real World entities; the other four become actionable hooks while retaining their original kind, full wording, generator edition, and source scene. A possible consequence remains a possibility—only scene resolution may record an actual world consequence. Run `npm run eval:in-play-material` for the bounded authority fixture.
