...
AI operations are explicit preparations. Their outputs use strict schemas, remain non-stored at the provider, cite authorized campaign evidence when derived from play, snapshot their campaign session where relevant, and enter a human review state. Creative campaign-opening generation is bounded differently because no campaign evidence exists yet: it accepts one GM premise as untrusted inspiration, produces a fixed typed private draft, and establishes no world truth until a GM edits and saves it. Character-concept generation is likewise a private draft boundary: it receives only the player-visible campaign foundation, must copy existing faction, location, and NPC identities, returns exactly three editable possibilities, and cannot create or revise a character. Generated house-rule wording is persisted before editing; unchanged acceptance, edited acceptance, and rejection preserve the original proposal and decision reason as versioned evaluation evidence. Only players save their own characters. Only GMs establish campaign foundations, decide canon, publish recaps, record house rules, or accept faction motion. Visibility is enforced for the campaign, all GMs, or named character seats. Administrative ownership is independent from knowledge access.
