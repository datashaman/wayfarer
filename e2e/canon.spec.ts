...
test('the owner reviews a cited canon proposal and returns to its source', async ({ page }) => {
  ...
  await expect(evaluation.locator('.evaluation-surface')).toHaveCount(12)
  await expect(evaluation.locator('.evaluation-surface').filter({ hasText: 'Adventure continuation' })).toBeVisible()
