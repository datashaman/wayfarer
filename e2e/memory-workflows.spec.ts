...
test('a GM edits and publishes a recap and resolves continuity inline', async ({ browser, page }) => {
  ...
  await intelligence.getByLabel('Title').fill('Lantern searches')
  await intelligence.getByLabel('Source rule').fill('Core perception rule.')
  await intelligence.getByLabel('Interpretation').fill('Bright light reveals old marks.')
  await intelligence.getByLabel('Table ruling').fill('Careful searches gain advantage.')
