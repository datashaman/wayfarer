...
const campaignIntelligence = {
  ...
  async draftAdventureContinuation({ characters, world, threads }) { return { title: 'Before the salvagers arrive', framing: 'Hooks scrape across Bell Square while Sister Corda bars the opened stair.', stakes: 'The drowned archive will be stripped bare before dawn.', question: 'Who claims the stair first?', characterIds: characters.map((item) => item.id), locationIds: [world.locations[0].id], npcIds: [world.npcs.find((item) => item.name === 'Sister Corda')?.id ?? world.npcs[0].id], clues: ['The hooks carry fresh bell-metal.', 'Someone opened the eastern sluice.', 'Corda recognises the salvage mark.'], complications: ['The square begins to flood again.', 'The Tidebound demand the stair.', 'A stolen name answers from below.'], sessionQuestions: ['Who reaches the archive first?', 'What will Corda trade for help?', 'Who controls the stair by dawn?'], threadIds: threads.slice(0, 3).map((item) => item.id), generatorVersion: campaignIntelligence.version } },
}
