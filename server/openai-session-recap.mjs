  ...
  return createSessionRecapGenerator({
    ...
    async generate({ messages, acceptedCanon }) {
      const response = await openai.responses.create({
        ...
        input: JSON.stringify({ transcript: messages.map(({ id, roomName, senderName, text, sentAt }) => ({ id, roomName, senderName, text, sentAt })), acceptedCanon }),
