...
function inferenceDuration(durationMs: number | null) {
  if (durationMs === null) return 'No timing yet'
  return durationMs < 1_000 ? `${durationMs} ms average` : `${(durationMs / 1_000).toFixed(1)} s average`
}

function EvaluationLedger({ dashboard, onNavigate }: { dashboard: AiEvaluationDashboard; onNavigate: (target: CanonLedgerTarget) => void }) {
  const { readiness, versions, surfaces, runs, alerts } = dashboard
  ...
  return <section className="canon-section evaluation-ledger" aria-labelledby="ai-evaluation-heading">
    ...
    <div className="evaluation-subsection"><h4>Surface evidence</h4><p className="evaluation-subsection-copy">Current edition, adversarial proof, and private runtime health for every production assistant.</p><div className="evaluation-surfaces">{surfaces.map((surface) => { const watch = surface.runtime.failed > 0; const state = !surface.version ? 'Unconfigured' : watch ? 'Watch' : surface.runtime.total ? 'Observed' : 'Untried'; const stateClass = watch ? 'evaluation-version__state evaluation-version__state--watch' : state === 'Observed' ? 'evaluation-version__state' : `evaluation-version__state evaluation-surface__state--${state === 'Untried' ? 'waiting' : 'absent'}`; return <article className="evaluation-surface" key={surface.id}><div className="evaluation-surface__identity"><span>{surface.authority}</span><strong>{surface.label}</strong><small>{surface.version ?? 'No generator configured'}</small></div><div className="evaluation-surface__proof"><span>Live proof</span><strong>{surface.liveCheck ? `${surface.liveCheck.passed}/${surface.liveCheck.total}` : '—'}</strong><small>{surface.liveCheck ? new Date(surface.liveCheck.createdAt).toLocaleDateString() : surface.evaluationCommand}</small></div><div className="evaluation-surface__runtime"><span>Runtime</span><strong>{evaluationPercent(surface.runtime.successRate)}</strong><small>{surface.runtime.total} {surface.runtime.total === 1 ? 'trace' : 'traces'} · {inferenceDuration(surface.runtime.averageDurationMs)}</small>{surface.runtime.averageInputUnits !== null && <small>{surface.runtime.averageInputUnits} in · {surface.runtime.averageOutputUnits ?? '—'} out</small>}</div><span className={stateClass}>{state}</span></article> })}</div></div>
  ...
  return (
    <div className="player-row">
      ...
      <div className="player-copy">
        ...
        {participant.characterName && participant.playerName && <small className="player-seat-name">{participant.playerName}</small>}
      </div>
    </div>
  )
}
  ...
  return (
    <article className={`message ${highlighted ? 'message--highlighted' : ''}`} id={`message-${message.id}`}>
      ...
      <div className="message-body">
        <div className="message-meta"><strong>{message.senderName}</strong>{message.characterName && message.playerName && <span>{message.playerName}</span>}<time>{time}</time></div>
        <p>{message.text}</p>
      </div>
    </article>
  )
