...
import type { CampaignWorld, TableSession, WorldEntityState } from './types/protocol'

function CurrentTruth({ type, state }: { type: 'faction' | 'location' | 'npc' | 'hook'; state?: WorldEntityState | null }) {
  if (!state) return null
  const truths = type === 'location' ? [['Control', state.ownership], ['Danger', state.danger]]
    : type === 'hook' ? [['Situation', state.situation], ['State', state.status]]
      : [['Goal', state.goal], ['Relationship', state.relationship]]
  return <aside className={`world-current-state ${state.status === 'resolved' ? 'is-settled' : ''}`}><span>Current truth</span><dl>{truths.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><blockquote><small>Pressure still moving</small>{state.pressure}</blockquote></aside>
}
