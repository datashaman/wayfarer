...
const openingQuestionSuggestion = 'The bell rope pulls taut in Iria’s hand. Who cuts it—and who stops them?'
const possibilitySuggestions = {
  clues: 'The bell rope was cut once before, then carefully spliced.',
  complications: 'The Tidebound arrive asking the party to let the town drown.',
  sessionQuestions: 'Who rang the bell before the town returned?',
}
...
export function SceneFolio({ session, suppliedContext, onContext, onOpenRoom, onClose }: {
  ...
  const usePossibilitySuggestions = () => draft && setDraft({
    ...draft,
    clues: draft.clues.map((item) => item.trim() ? item : possibilitySuggestions.clues),
    complications: draft.complications.map((item) => item.trim() ? item : possibilitySuggestions.complications),
    sessionQuestions: draft.sessionQuestions.map((item) => item.trim() ? item : possibilitySuggestions.sessionQuestions),
  })
  ...
  const missingPreparation = !draft ? [] : [
    !draft.title.trim() ? 'Name the opening moment' : '',
    !draft.framing.trim() ? 'Describe what the characters see happening' : '',
    !draft.stakes.trim() ? 'Say what changes if nobody acts' : '',
    !draft.question.trim() ? 'Name the first choice' : '',
    !draft.characterIds.length ? 'Choose at least one character' : '',
    !draft.locationIds.length ? 'Choose at least one location' : '',
    !draft.npcIds.length ? 'Choose at least one NPC' : '',
    !draft.clues.every((item) => item.trim()) ? 'Complete every clue or discovery' : '',
    !draft.complications.every((item) => item.trim()) ? 'Complete every likely complication' : '',
    !draft.sessionQuestions.every((item) => item.trim()) ? 'Complete every session question' : '',
  ].filter(Boolean)
  const draftComplete = Boolean(draft && missingPreparation.length === 0)
    ...
    <aside className="scene-folio">
      ...
      <div className="scene-folio__body">
        ...
        {pending === 'load' ? <p className="folio-loading">Reading the table’s place in the story…</p> : !context?.openingCrisis ? <section className="scene-prerequisite"><CircleDot size={22} /><span className="eyebrow">The world comes first</span><h2>Establish the campaign opening</h2><p>A scene needs the crisis, pressure, and stakes kept in the World folio.</p></section> : !context.characters.length ? <section className="scene-prerequisite"><CircleDot size={22} /><span className="eyebrow">The party comes first</span><h2>Someone must take a seat</h2><p>Create at least one character before framing the opening scene.</p></section> : context.activeScene ? <section className="scene-active">
          ...
          <div className="scene-field-label"><label htmlFor="scene-question">What demands the first choice?</label><button type="button" className="folio-small-action" onClick={() => setDraft({ ...draft, question: openingQuestionSuggestion })} disabled={Boolean(draft.question.trim())}>Use suggestion</button></div><textarea id="scene-question" value={draft.question} onChange={(event) => setDraft({ ...draft, question: event.target.value })} maxLength={500} placeholder={openingQuestionSuggestion} />
          ...
          <section className="scene-preparation-group"><div className="scene-preparation-heading scene-preparation-heading--action"><div><span className="eyebrow">Prepared possibilities</span><p>Offer things the characters can discover and pressures that may enter. None dictate what happens.</p></div><button type="button" className="folio-button" onClick={usePossibilitySuggestions} disabled={draft.clues.every((item) => item.trim()) && draft.complications.every((item) => item.trim()) && draft.sessionQuestions.every((item) => item.trim())}>Use suggested possibilities</button></div>
            {([['clues', 'Clues or discoveries', 'What can become known?', possibilitySuggestions.clues], ['complications', 'Likely complications', 'What trouble may enter?', possibilitySuggestions.complications], ['sessionQuestions', 'Questions for the session', 'What are you playing to find out?', possibilitySuggestions.sessionQuestions]] as const).map(([field, heading, label, placeholder]) => <div className="scene-preparation-list" key={field}><div><strong>{heading}</strong><button type="button" className="folio-button" onClick={() => addListItem(field)} disabled={draft[field].length >= 8}><Plus size={14} />Add</button></div>{draft[field].map((item, index) => <label key={index}><span>{label}</span><span className="scene-preparation-list__field"><textarea value={item} onChange={(event) => updateList(field, index, event.target.value)} maxLength={500} placeholder={placeholder} aria-label={`${heading} ${index + 1}`} /><button type="button" className="icon-button" onClick={() => removeListItem(field, index)} disabled={draft[field].length === 1} aria-label={`Remove ${heading.toLowerCase()} ${index + 1}`}><Trash2 size={14} /></button></span></label>)}</div>)}
          </section>
          ...
          <footer><div className="scene-readiness" role="status"><strong>{missingPreparation.length ? `${missingPreparation.length} ${missingPreparation.length === 1 ? 'thing' : 'things'} before this scene can be saved` : hasUnsavedChanges ? 'Ready to save' : `Preparation revision ${context.preparation?.revision ?? 0} is kept`}</strong>{missingPreparation.length ? <ul>{missingPreparation.map((item) => <li key={item}>{item}</li>)}</ul> : <span>{hasUnsavedChanges ? 'Save this preparation first. Then Begin play will put the opening moment into the transcript.' : 'The preparation is durable. Begin play when everyone is ready.'}</span>}</div><button type="submit" className={hasUnsavedChanges ? 'primary-action' : 'folio-button'} disabled={!draftComplete || !hasUnsavedChanges || pending === 'save'}>{pending === 'save' ? 'Keeping preparation…' : context.preparation ? 'Save revision' : 'Save preparation'}</button><button type="button" className={!hasUnsavedChanges && context.preparation ? 'primary-action' : 'folio-button'} onClick={establish} disabled={!draftComplete || hasUnsavedChanges || !context.preparation || pending === 'start'}>{pending === 'start' ? 'Opening the scene…' : 'Begin play'}</button></footer>
        </form>}
