...
test('continuity briefs are owner-private, canon-aware, cited, and rateable', async (t) => {
  ...
  assert.equal((await json(`${origin}/api/campaign/ai/evaluation`, { headers: memberAuthorization })).status, 403)
  const evaluation = await json(`${origin}/api/campaign/ai/evaluation`, { headers: ownerAuthorization })
  assert.equal(evaluation.status, 200)
  assert.equal(evaluation.body.dashboard.readiness.metrics.canon.total, 1)
  assert.equal(evaluation.body.dashboard.readiness.metrics.continuity.usefulRate, 1)
  assert.deepEqual(evaluation.body.dashboard.versions.map((version) => version.version), ['fixture-v1', 'fixture-continuity-v1'])
